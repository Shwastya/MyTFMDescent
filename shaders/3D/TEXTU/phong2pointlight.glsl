...
void main()
{
    ...
    if (v_IsMaterial > 0)
    {
        ...
        vec3 lightDir = normalize(u_pointLight.position - v_fragPos); // pl

        float diff =    max(dot(norm, lightDir), 0.0);
        vec3 diffuse =  diff * u_material.diffuse * u_pointLight.diffuse;
    ...
    else
    {
        ...
        vec3 ambient = albedo * u_pointLight.ambient;
        ...
        vec3 lightDir = normalize(u_pointLight.position - v_fragPos); // pl
        float diff =    max(dot(norm, lightDir), 0.0);
        vec3 diffuse =  diff * albedo * u_pointLight.diffuse;
