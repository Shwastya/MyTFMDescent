...
namespace MHelmet
{
    ...
    void Geometry::uploadData(const float* positions, const float* uvs, const float* normals, const uint32_t* indices) 
    {
       ...
       // memset(biTangents, 0.0f, length * sizeof(float));
        ...
        std::cout << "numero de floats positions  " <<  _nVertices * 3 << std::endl;

        std::cout << "numero de floats UVS  " << _nVertices * 2 << std::endl;

        std::cout << "numero de floats NORMALS  " << _nVertices * 3 << std::endl;

        std::cout << "numero de floats TANGENTS  " << _nVertices * 3 << std::endl;

        std::cout << "numero de floats BITANGENTS  " << _nVertices * 3 << std::endl;

        std::cout << "numero de elementos " << _nElements << std::endl;

        std::cout << "numero de vertices " << _nVertices << std::endl;

        size_t floatSize = (size_t)_nVertices * 3 + (size_t)_nVertices * 2 + (size_t)_nVertices * 3 + (size_t)_nVertices * 3 + (size_t)_nVertices * 3;

        std::cout << "numero de FLOATS total " << floatSize << std::endl;

        float* test = new float[floatSize];
